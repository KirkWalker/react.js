...
import {ResetPasswordPost} from '../services/ForgotPassword';
...
class ResetPassword extends Component {
    ...
    sendemail() {
        ...
        if(errorMessage == '') {
            ...
            ResetPasswordPost(this.state.password, this.state.token).then((result) => {
                var msg = '<p>'+strings.password_reset+' <a href="/login">'+strings.click_login+'</a></p>';
    ...
    handleResponse(_responseJson,_msg) {
        ...
        if (_responseJson.status == 0) {
            this.setState({error: '<div class="alert alert-danger">' + _responseJson.msg + '</div>'});
        } else {
